...
modules.each do |m|
  ...
  if m[0] != "PACKAGE"
    ...
    if m[1] == "custom" && m[0] == "linux"
      ...
      e['version'] = globalOptions[:uboot] + "-custom-" + m[1][0, 7]
    end
  end
  ...
  modules.each do |m|
    if m[0] != "PACKAGE"
      if m[0] == "linux" and m[1] == "custom"
        ...
        m[0] = "uboot"
        new_name = globalOptions[:uboot] + "-custom-" + m[1][0, 7]
        %x(mv #{globalOptions[:dir]}/licenses/muboot-#{m[1]} #{globalOptions[:dir]}/licenses/uboot-#{new_name})
        m[1] = new_name
      end
    end
