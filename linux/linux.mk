...
define LINUX_POST_PATCH_FUNC
	...
	mkdir -p $(STAGING_DIR)/usr/src/linux
	( cd $(@D); \
...
define LINUX_POST_INSTALL_IMAGES_FUNC_IMPL
	...
	xz --force --check=none --lzma2=preset=6e,dict=64KiB < $(BINARIES_DIR)/mscc-linux-$(1).bin > $(BINARIES_DIR)/mscc-linux-$(1).bin.xz
	gzip < $(BINARIES_DIR)/mscc-linux-$(1).bin > $(BINARIES_DIR)/mscc-linux-$(1).bin.gz
	mkdir -p $(STAGING_DIR)/usr/src/mscc-linux-$(1)
	if test -e "$(@D)/Module.symvers" ; then \
...
define LINUX_POST_INSTALL_IMAGES_FUNC
	$(call LINUX_POST_INSTALL_IMAGES_FUNC_IMPL,kernel)
endef
